...
const ShowDetalleFacturaProveedor = () => {
    const idDetalle = '9FFFC55A-186F-4D62-88B4-78B43B2948C1';
    const url = `https://localhost:7289/api/productos/${idDetalle}/historicoPrecios`;   //Remplazar URL cuando este la api completa
...
console.log(idDetalle)
    ...
    const openModal = (op, detalleId, facturaId, productoId, producto, cantidad, precio, iva, subtotal, total) => {
        setDetalleId('');
        setFacturaId('');
        setProductoId('');
        setProducto('');
    ...
    const validar = () => {
        ...
        var metodo;
        if (idDetalle.trim() === '') {
            show_alerta('Escriba el id del detalle', 'warning');
        } else if (facturaId.trim() === '') {
            show_alerta('Escriba el id de la factura', 'warning');
            ...
            show_alerta('Escriba el IVA del producto', 'warning');
        } else if (subtotal.trim() === '') {
            show_alerta('Escriba el subtotal del producto', 'warning');
        } else if (total.trim() === '') {
            show_alerta('Escriba el total del producto', 'warning');
        } else {
            if (operation === 1) {
                parametros = {
    ...
    await axios({ method: metodo, url: URL, data: parametros })
        ...
        const deleteDetalle = (detalleId, productoId) => {
        const MySwal = withReactContent(Swal);

        MySwal.fire({
        title: `¿Seguro desea eliminar este detalle factura proveedor ${detalleId}?`,
        icon: 'question',
        ...
        if (result.isConfirmed) {
            try {
            await axios.delete(`${URL}/${detalleId}`);
            show_alerta('Detalle factura proveedor eliminado exitosamente', 'success');
            getDetalles();
            } catch (error) {
            show_alerta('Error al eliminar el detalle factura proveedor', 'error');
            console.error(error);
            }
        } else {
            show_alerta('El detalle factura proveedor no fue eliminado', 'info');
        }
        });
    }
        ...
        <div className='container-fluid'>
            ...
            <div className='col-md-4 offset-4'>
                ...
                <button onClick={() => openModal(1)} className='btn btn-dark' data-bs-toggle='modal' data-bs-target='#modalProductos'>
                    <i className='fa-solid fa-circle-plus'></i> Añadir Detalle
                </button>
                </div>
            </div>
            </div>
                ...
                <table className='table table-bordered'>
                    ...
                    {detalles.map((detalle, i) => (
                        ...
                        <td>
                            <button
                            onClick={() =>
                                openModal(2, detalle.detalleId, detalle.facturaId, detalle.productoId, detalle.producto, detalle.cantidad, detalle.precio, detalle.iva, detalle.subtotal, detalle.total)
                            }
                            ...
                            &nbsp;
                            <button onClick={() => deleteDetalle(detalle.detalleId, detalle.productoId)} className="btn btn-danger">
        ...
        <div id='modalProductos' className='modal fade' aria-hidden='true'>
            ...
            <div className='modal-content'>
                ...
                <div className='input-group mb-3'>
                    ...
                    placeholder='Id del Detalle'
                    value={detalleId}
                    onChange={(e) => setDetalleId(e.target.value)}
                    ></input>
                </div>
                <div className='input-group mb-3'>
                    <span className='input-group-text'>
