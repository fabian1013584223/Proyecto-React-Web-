...
const ShowMetodoPago = () => {
  const url = 'https://localhost:7284/api/Categorias';
  const [metodos, setMetodos] = useState([]);
  const [fechadepago, setFechaDePago] = useState('');
  const [IdMetodoPago, setIdMetodoPago] = useState('');
  const URL = `https://localhost:7284/api/Categorias/${IdMetodoPago}`; 
  ...
  const openModal = (op, IdMetodoPago, fechadepago) => {
    setIdMetodoPago('');
    setFechaDePago('');
    setOperation(op);
    if (op === 1) {
      setTitle('Registrar Metodo de pago');
    } else if (op === 2) {
      setTitle('Editar Metodo de pago');
      setIdMetodoPago(IdMetodoPago);
  ...
  const validar = () => {
    ...
    var metodo;
    if (fechadepago.trim() === '') {
      show_alerta('Digite la fecha de pago', 'warning');
    } else {
      if (operation === 1) {
        parametros = {
          fechadepago: fechadepago.trim(),
        };
        metodo = 'POST';
      } else {
        parametros = {
  ...
  const deleteMetodos = (IdMetodoPago, fechadepago) => {
    const MySwal = withReactContent(Swal);

    MySwal.fire({
      title: `¿Seguro desea eliminar esta fecha de pago: ${fechadepago}?`,
      icon: 'question',
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`${url}/${IdMetodoPago}`);

          show_alerta('Metodo de pago eliminado exitosamente', 'success');
          getMetodos();
        } catch (error) {
          show_alerta('Error al eliminar el metodo de pago', 'error');
          console.error(error);
        }
      } else {
        show_alerta('El metodo de pago no fue eliminado', 'info');
      }
    });
  };
  ...
  return (
    <div className='App'>
        ...
        <div className='row mt-3'>
          <div className='col-md-4 offset-4'>
            <div className='d-grid mx-auto'>
              <button onClick={() => openModal(1)} className='btn btn-dark' data-bs-toggle='modal' data-bs-target='#modalMetodos'>
              ...
              <table className='table table-bordered'>
                ...
                <tbody className='table-group-divider'>
                  {metodos.map((metodo, i) => (
                    <tr key={metodo.IdMetodoPago}>
                      <td>{i + 1}</td>
                      <td>{metodo.IdMetodoPago}</td>
                      <td>{metodo.fechadepago}</td>
                      <td>
                        <button
                          onClick={() => openModal(2, metodo.IdMetodoPago, metodo.fechadepago)}
                          className='btn btn-warning'
                          data-bs-toggle='modal'
                          data-bs-target='#modalMetodos'
                        >
                          <i className='fa-solid fa-edit'></i>
                        </button>
                        &nbsp;
                        <button onClick={() => deleteMetodos(metodo.IdMetodoPago, metodo.fechadepago)} className="btn btn-danger">
      ...
      <div id='modalMetodos' className='modal fade' aria-hidden='true'>
