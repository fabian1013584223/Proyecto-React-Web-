...
const ShowFacturaCliente = () => {
    const url = 'https://localhost:7284/api/facturasCliente';
    ...
    const URL = `https://localhost:7284/api/facturasCliente/${facturaClienteId}`; 
    ...
    const getFacturaCliente = async () => {
        const respuesta = await axios.get(url);
        setFacturaCliente(respuesta.data);
    };

    const openModal = (op, facturaClienteId, fechaGeneracion) => {
        setFacturaClienteId('');
        setFechaGeneracion('');
        setOperation(op);
        if (op === 1) {
        ...
        setFacturaClienteId(facturaClienteId);
    ...
    const validar = () => {
        ...
        var parametros;
        var metodo;
        if (fechaGeneracion.trim()=== ''){
        show_alerta('escriba la fecha')
        }
        else {
        if (operation === 1) {
            parametros = {
                fechaGeneracion:fechaGeneracion.trim(),
            };
            metodo = 'POST';
        } else {
            parametros = {
    ...
    const deleteFactura = (facturaClienteId,fechaGeneracion) => {
        const MySwal = withReactContent(Swal);

        MySwal.fire({
        title: `¿Seguro desea eliminar esta factura ${fechaGeneracion}?`,
        icon: 'question',
        ...
        if (result.isConfirmed) {
            ...
            await axios.delete(`${url}/${facturaClienteId}`);
    ...
    <table className='table table-bordered'>
        ...
        {facturaCliente.map((facturaCliente, i) => (
            <tr key={facturaCliente.facturaClienteId}>
            <td>{i + 1}</td>
            <td>{facturaCliente.facturaClienteId}</td>
            <td>{facturaCliente.fechaGeneracion}</td>
            <td>
                <button
                onClick={() => openModal(2, facturaCliente.facturaClienteId, facturaCliente.fechaGeneracion)}
            ...
            <button
                onClick={() => deleteFactura(facturaCliente.facturaClienteId, facturaCliente.fechaGeneracion)}
        ...
        <div id='modalFactura' className='modal fade' aria-hidden='true'>
            ...
            <div className='modal-content'>
                ...
                <input
                    ...
                    placeholder='FacturaCliente'
                    value={facturaClienteId}
                    onChange={(e) => setFacturaClienteId(e.target.value)}
                    ></input>
                </div>
                <div className='input-group mb-3'>
                    <span className='input-group-text'>
