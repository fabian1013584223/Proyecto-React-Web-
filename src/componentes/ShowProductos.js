...
const ShowProductos = () => {
const iD='4F9A7CDB-C6DD-4C1C-8CA2-88F34C913C54'
  const url = `https://localhost:7284/api/categorias/${iD}/productos`;
  const [productos, setProductos] = useState([]);
  const [productoId, setProductoId] = useState('');
  const URL=`https://localhost:7284/api/productos/${iD}/productos${productoId}`;
...
console.log(productoId)
  ...
  const openModal = (op, productoId, nombre, cantidad, precio, estado, lugar) => {
    setProductoId('');
    setNombre('');
    setCantidad('');
    setPrecio('');
  ...
  const deleteProducto = (pId, nombre) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`${URL}/${productoId}`);
                  ...
                  {productos.map((producto, i) => (
                    <tr key={producto.productoId}>
                      ...
                      <td>
                        <button
                          onClick={() =>
                            openModal(2, producto.productoId, producto.nombre, producto.cantidad, producto.precio, producto.estado, producto.lugar)
                          }
                        ...
                        &nbsp;
                        <button onClick={() => deleteProducto(producto.productoId, producto.nombre)} className="btn btn-danger">
