...
const iD='9FFFC55A-186F-4D62-88B4-78B43B2948C1'
    const url = `https://localhost:7289/api/productos/${iD}/historicoPrecios`;
    const [precios, setPrecios] = useState([]);
    const [historicoPrecios, setHistoricoPrecios] = useState('');
    const URL=`https://localhost:7289/api/historicoPrecios`;
...
console.log(precioId)
    ...
    const openModal = (op,precioId,  productoId, precioCompra, precioVenta, precioDescuento,fechaDescuento, estado) => {
        setPrecioId('');
        setProductoId('');
        setPrecioCompra('');
        setPrecioVenta('');
        ...
        setOperation(op);
        if (op === 1) {
        setTitle('Registrar Precio');
        } else if (op === 2) {
        setTitle('Editar Precio');
        setPrecioId(precioId);
        setProductoId(productoId);
        setPrecioCompra(precioCompra);
        setPrecioVenta(precioVenta);
        setPrecioDescuento(precioDescuento);
        setFechaDescuento(fechaDescuento);
        setEstado(estado);
        }
    ...
    const validar = () => {
        ...
        var metodo;
        if (precioId.trim() === '') {
        show_alerta('Escriba el id del precio', 'warning');
        } else if (productoId.trim === '') {
            show_alerta('Escriba el id del producto', 'warning');
        } else if (precioCompra.trim === '') {
        show_alerta('Escriba la predio de compra del producto', 'warning');
        } else if (precioVenta.trim === '') {
        show_alerta('Escriba el precio de venta del producto', 'warning');
        } else if (precioDescuento === '') {
            show_alerta('Escriba el precio de descuento del producto', 'warning');
        } else if (fechaDescuento === '') {
            show_alerta('Escriba la fecha de descuento del producto', 'warning');
        } else if (estado.trim() === '') {
        show_alerta('Escriba el estado del producto', 'warning');
        } else {
        if (operation === 1) {
        parametros = {
            PrecioId: precioId.trim(),
    ...
    await axios({ method: metodo, url: URL, data: parametros })
        ...
        const deletePrecio = (precioId, productoId) => {
        const MySwal = withReactContent(Swal);

        MySwal.fire({
        title: `¿Seguro desea eliminar este producto ${productoId}?`,
        icon: 'question',
        ...
        if (result.isConfirmed) {
            try {
            await axios.delete(`${URL}/${precioId}`);
            show_alerta('Precio eliminado exitosamente', 'success');
            getPrecios();
            } catch (error) {
            show_alerta('Error al eliminar el precio', 'error');
            console.error(error);
            }
        } else {
            show_alerta('El Precio no fue eliminado', 'info');
        }
        });
    }
            ...
            <div className='col-12 col-lg-8 offset-0 offset-lg-12'>
                ...
                <table className='table table-bordered'>
                    ...
                    {precios.map((precio, i) => (
                        ...
                        <td>{precio.productoId}</td>
                        <td>{precio.precioCompra}</td>
                        <td>{precio.PrecioVenta}</td>
                        <td>{precio.precioDescuento}</td>
                        <td>{precio.fechaDescuento}</td>
                        <td>{precio.estado}</td>
                        <td>
                            <button
                            onClick={() =>
                                openModal(2, precio.precioId, precio.productoId, precio.precioCompra, precio.PrecioVenta, precio.precioDescuento, precio.FechaDescuento, precio.Estado)
                            }
                            ...
                            &nbsp;
                            <button onClick={() => deletePrecio(precio.PrecioId, precio.productoId)} className="btn btn-danger">
        ...
        <div id='modalProductos' className='modal fade' aria-hidden='true'>
            ...
            <div className='modal-content'>
                ...
                <input
                    ...
                    id='precioDescuento'
                    className='form-control'
                    placeholder='Precio de descuento del producto'
                ...
                <div className='input-group mb-3'>
                    ...
                    id='fechaDescuento'
                    className='form-control'
                    placeholder='Fecha de descuento del producto'
